...
//            communication_cv->wait(lock);
                communication_cv->wait(lock, [&] {
                    return !communication_queue.empty() || rdma_mg->handler_is_finish.load();
                });
...
//            fflush(stdout);
                ...
                DynamicCompoundKey primary_key(primary_key_buffer,
                                               storage_manager_->tables_[received_rdma_request.content.tuple_info.table_id]->GetSchema());
                switch (received_rdma_request.command) {
                    case tuple_read_2pc:
                        memcpy(primary_key_buffer, &received_rdma_request.content.tuple_info.primary_key,
                               sizeof(uint64_t));
...
//                fflush(stdout);
                    ...
                    rdma_mg->RDMA_Write_xcompute(local_mr, received_rdma_request.buffer, received_rdma_request.rkey,
                                                 sizeof(RDMA_ReplyXCompute) + tuple_size, target_node_id, qp_id, false);
...
//            communication_queue->command = invalid_command_;
    ...
    private:
        ...
        virtual void ProcessQuery() {
            ...
            std::cout << "Node" << RDMA_Manager::Get_Instance()->node_id << "execute_count=" << total_count_
                      << ", abort_count="
...
//            assert(false);
                        ...
                        if (is_finish_ == true) {
                            ...
                            total_abort_count_.fetch_add(abort_count);PROFILE_TIME_END(thread_id, TXN_EXECUTE);
//            txn_manager->CleanUp();
                            return;
                        ...
                        while (procedures[tuple->type_]->Execute(tuple, ret) == false) {
                            ...
                            if (is_finish_ == true) {
                                total_count_.fetch_add(count);
                                total_abort_count_.fetch_add(abort_count);PROFILE_TIME_END(thread_id,
                                                                                           TXN_ABORT);PROFILE_TIME_END(
...
//            printf("Transaction finished for thread %zu\n", thread_id);
                    ...
                    ++count;PROFILE_TIME_END(thread_id, TXN_EXECUTE);
                    if (count % 10000 == 0) {
                        printf("Node %u Thread %zu finished %d\n", default_gallocator->rdma_mg->node_id, thread_id,
                               count);
                        fflush(stdout);
                    }
                    if (is_finish_ == true) {
