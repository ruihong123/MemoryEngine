...
//        uint64_t commit_ts = GlobalTimestamp::FetchAddMonotoneTimestamp();
        ...
        for (auto iter : sorted_access){
          ...
          page_buff = ((ibv_mr*)handle->value)->addr;
            ...
            page_buff = handle->value;
                ...
                uint64_t rts = access->access_global_record_->GetRTS();
                uint64_t wts = access->access_global_record_->GetWTS();
                //TODO； TO rules can allow the read only transaction to read the record with larger wts.
                if (rts > start_timestamp_ || wts > start_timestamp_){
                    if (rts < start_timestamp_){
                        // ignore the update
                        access->txn_local_tuple_->CopyFrom(access->access_global_record_);
                    }else{
                        AbortTransaction();
                        return false;
                    }
                    ...
                    access->txn_local_tuple_->PutWTS(start_timestamp_);
                }
          ...
          page_buff = ((ibv_mr*)handle->value)->addr;
            ...
            page_buff = handle->value;
                ...
                if (wts > start_timestamp_){
                    ...
                    access->txn_local_tuple_->PutRTS(start_timestamp_);
                }
            }
        ...
        for (size_t i = 0; i < access_list_.access_count_; ++i) {
            ...
            if (access_type == READ_WRITE || access_type == INSERT_ONLY) {
                ...
                access->access_global_record_->PutWTS(start_timestamp_);
            }
