    ...
    public:
        ...
        RecordSchema* CreatePrimaryIndexSchema() {
            ...
            if (index_schema_ptr == nullptr) {
                ...
                // Compressed mode: single UINT64 key column
                std::vector<DSMEngine::ColumnInfo*> columns;
                columns.push_back(new DSMEngine::ColumnInfo("key", DSMEngine::ValueType::UINT64));
                columns.push_back(new DSMEngine::ColumnInfo("gptr", DSMEngine::ValueType::UINT64));
                index_schema_ptr->BulkloadColumns(columns);
                size_t column_ids[] = {0};
                index_schema_ptr->SetPrimaryColumns(column_ids, 1);
#else
                // Multi-field mode: extract primary key columns from table schema
                ...
                for (size_t i = 0; i < key_num; ++i) {
                    ...
                    columns.push_back(new DSMEngine::ColumnInfo(
                        schema_ptr_->GetColumnName(col_id), schema_ptr_->GetColumnType(col_id)));
                }
                columns.push_back(new DSMEngine::ColumnInfo("gptr", DSMEngine::ValueType::UINT64));
                index_schema_ptr->BulkloadColumns(columns);
        ...
        void Init(size_t table_id, RecordSchema* schema_ptr, DDSM* ddsm) {
            ...
            secondary_count_ = 0;
            //    secondary_indexes_ = nullptr;
            // the index init shall be deprecated, since we can init the index in the constructor. If the index does not
            // need init we can tell that by the number of constructor arguments.
        ...
        GlobalAddress SearchPriIndex(const DynamicCompoundKey key) {
            ...
            if (find) {
                memcpy(&tuple_gaddr, key_value_pair + primary_index_->index_scheme_ptr->GetPrimaryKeyLength(),
        ...
        // Allocate the tuple from new page, if there is a cached handles list, we need to consider whether the latch
        // has already been
        // acquired.
            ...
            if (locked_handles_ != nullptr && g_addr != nullptr) {
                ...
                if (locked_handles_->find(cacheline_g_addr) == locked_handles_->end()) {
                    ...
                    if (locked_handles_->at(cacheline_g_addr).second == READ_ONLY) {
                        //                  default_gallocator->SELCC_Lock_Upgrade(page_buffer, cacheline_g_addr,
                        //                  handle);
                ...
                *g_addr = gallocator->Allocate_Remote(Regular_Page);
                ...
                //          (8ull*schema_ptr_->GetRecordTotalSize() +1);
                uint64_t cardinality =
                    DataPage::calculate_cardinality(kLeafPageSize, schema_ptr_->GetRecordTotalSize());
