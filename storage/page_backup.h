...
#ifndef NDEBUG
        // record first 8 bytes of the key
        // this is used for debugging purpose, to check whether the key is correct.
        uint64_t this_key = 0;
...
//        uint8_t rear_version;
    ...
    struct InternalPageDynamic {
        Header_Index<void> hdr;
        // raw bytes follow hdr in memory:
        //   keys:  char keys[hdr.last_index+1][schemaSize];
        //   ptrs:  GlobalAddress ptrs[hdr.last_index+2];
        bool search(const void* target_key, SearchResultD& out, RecordSchema* schema);
        bool insert(const void* new_key, GlobalAddress ptr, RecordSchema* schema);
      };
      struct LeafPageDynamic {
        Header_Index<void> hdr;
        // raw bytes follow hdr in memory:
        //   keys:  char keys[hdr.last_index+1][schemaSize];
        //   ptrs:  GlobalAddress ptrs[hdr.last_index+2];
