...
void ExchPerfStatistics(ClusterConfig* config, ClusterSync* synchronizer, PerfStatistics* s);
...
int main(int argc, char* argv[]) {
    ...
    synchronizer.MasterBroadcast(storage_key, 16, storage_addr, TableDirectory::GetSerializeSize());
    ...
    if (TWOPHASECOMMIT) {
        auto func = std::bind(
            &SmallBankExecutor::ProcessQueryThread_2PC_Participant, (void*) &storage_manager, std::placeholders::_1);
        default_gallocator->rdma_mg->Set_message_handling_func(func, TwoPC);
    }

    // generate workload
    IORedirector redirector(gThreadCount);
    SmallBankSource sourcer(&smallbank_scale_params, &redirector, num_txn, 0, gThreadCount);
    sourcer.Start();

    IORedirector redirector1(gThreadCount);
    SmallBankSource sourcer1(&smallbank_scale_params, &redirector1, num_txn / 4, 0, gThreadCount);
    sourcer1.Start();
...

    {
        ...
        INIT_PROFILE_TIME(gThreadCount);
        SmallBankExecutor executor(&redirector, &storage_manager, gThreadCount, false);
        executor.Start();
        REPORT_PROFILE_TIME(gThreadCount);
    }
...

    {
        ...
        INIT_PROFILE_TIME(gThreadCount);
        SmallBankExecutor executor(&redirector1, &storage_manager, gThreadCount, LOGGING);
        executor.Start();
        REPORT_PROFILE_TIME(gThreadCount);
