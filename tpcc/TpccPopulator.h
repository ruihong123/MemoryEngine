...
//                         w_id <= scale_params_->ending_warehouse_; ++w_id) {
                    ...
                    if (scale_params_->partition_id_ == 0){
                        GenerateItemRecord(item_id, item_record);
...
//                    }
        ...
        private:
            ...
            void GenerateItemRecord(const int &item_id, ItemRecord *record) const {
...
#endif
                auto key = GetItemPrimaryKey(record_ptr->i_id_,
                                             storage_manager_->tables_[ITEM_TABLE_ID]->GetPrimaryIndexSchema());
//                printf("key %d tuple gaddr: %p\n", record_ptr->i_id_, tuple_gaddr);
//                fflush(stdout);
            ...
            void InsertWarehouseRecord(WarehouseRecord *record_ptr, Record *record_buf,
                ...
                storage_manager_->tables_[WAREHOUSE_TABLE_ID]->AllocateNewTuple(
                        tuple_data_, tuple_gaddr, handle, gallocator, nullptr);
//    GAddr data_addr = gallocator->Malloc(record_buf->GetRecordTotalSize());
                Record record_in_cache = Record(storage_manager_->tables_[WAREHOUSE_TABLE_ID]->GetSchema(),
                                                tuple_data_);
...
//      meta.next_delta_wts_ = 0;
                ...
                auto k = GetDistrictPrimaryKey(record_ptr->d_id_, record_ptr->d_w_id_,
                                               storage_manager_->tables_[DISTRICT_TABLE_ID]->GetPrimaryIndexSchema());
...
//      meta.next_delta_wts_ = 0;
                ...
                auto key = GetCustomerPrimaryKey(record_ptr->c_id_, record_ptr->c_d_id_,
                                                 record_ptr->c_w_id_,
                                                 storage_manager_->tables_[CUSTOMER_TABLE_ID]->GetPrimaryIndexSchema());
...
//      meta.next_delta_wts_ = 0;
                ...
                auto key = GetStockPrimaryKey(record_ptr->s_i_id_, record_ptr->s_w_id_,
                                              storage_manager_->tables_[STOCK_TABLE_ID]->GetPrimaryIndexSchema());
...
//      meta.next_delta_wts_ = 0;
                ...
                auto key = GetOrderPrimaryKey(record_ptr->o_id_, record_ptr->o_d_id_,
                                              record_ptr->o_w_id_,
                                              storage_manager_->tables_[ORDER_TABLE_ID]->GetPrimaryIndexSchema());
            ...
            void InsertNewOrderRecord(NewOrderRecord *record_ptr, Record *record_buf,
                ...
                storage_manager_->tables_[NEW_ORDER_TABLE_ID]->AllocateNewTuple(
                        tuple_data_, tuple_gaddr, handle, gallocator, nullptr);
                Record record_in_cache = Record(storage_manager_->tables_[NEW_ORDER_TABLE_ID]->GetSchema(),
                                                tuple_data_);
...
//      meta.next_delta_wts_ = 0;
                ...
                auto key = GetNewOrderPrimaryKey(record_ptr->o_id_, record_ptr->d_id_,
                                                 record_ptr->w_id_,
                                                 storage_manager_->tables_[NEW_ORDER_TABLE_ID]->GetPrimaryIndexSchema());
            ...
            void InsertOrderLineRecord(OrderLineRecord *record_ptr, Record *record_buf,
                ...
                storage_manager_->tables_[ORDER_LINE_TABLE_ID]->AllocateNewTuple(
                        tuple_data_, tuple_gaddr, handle, gallocator, nullptr);
                Record record_in_cache = Record(storage_manager_->tables_[ORDER_LINE_TABLE_ID]->GetSchema(),
                                                tuple_data_);
...
//      meta.next_delta_wts_ = 0;
                ...
                auto key = GetOrderLinePrimaryKey(record_ptr->ol_o_id_,
                                                  ...
                                                  storage_manager_->tables_[ORDER_LINE_TABLE_ID]->GetPrimaryIndexSchema());
...
//      meta.next_delta_wts_ = 0;
                ...
                auto key = GetHistoryPrimaryKey(record_ptr->h_c_id_,
                                                record_ptr->h_d_id_,
                                                record_ptr->h_w_id_,
                                                storage_manager_->tables_[HISTORY_TABLE_ID]->GetPrimaryIndexSchema());
            ...
            void InsertDistrictNewOrderRecord(DistrictNewOrderRecord *record_ptr,
                ...
                storage_manager_->tables_[DISTRICT_NEW_ORDER_TABLE_ID]->AllocateNewTuple(
                        tuple_data_, tuple_gaddr, handle, gallocator, nullptr);
                Record record_in_cache = Record(storage_manager_->tables_[DISTRICT_NEW_ORDER_TABLE_ID]->GetSchema(),
                                                tuple_data_);
...
//      meta.next_delta_wts_ = 0;
                ...
                auto key = GetDistrictNewOrderPrimaryKey(record_ptr->d_id_,
                                                         record_ptr->w_id_,
                                                         storage_manager_->tables_[DISTRICT_NEW_ORDER_TABLE_ID]->GetPrimaryIndexSchema());
