...
DSMEngine::Memory_Node_Keeper::Memory_Node_Keeper(bool use_sub_compaction,
    ...
    : pr_size(pr_s), usesubcompaction(use_sub_compaction)

{
  ...
  mem_phys_ids.reserve(rdma_mg->memory_nodes.size());
    ...
    while (iss >> mem_idx) {
      ...
      physical_regions.push_back(
          {mem_phys_ids[mem_idx], 0,
           0}); // base_ptr will be synchronized later via memcached
    }
    if (physical_regions.empty()) {
      // Identity fallback if no replica list provided
      if (rdma_mg->memory_nodes.count(logical_id))
        physical_regions.push_back({logical_id, 0});
    }
    if (!physical_regions.empty()) {
      rdma_mg->logical_groups[logical_id] =
  ...
  if (rdma_mg->logical_groups.empty()) {
    for (const auto &kv : rdma_mg->memory_nodes) {
      rdma_mg->logical_groups[kv.first] =
          LogicalGroup{{{kv.first, 0}}, /*bytes=*/0};
    }
  }
...
void Memory_Node_Keeper::server_communication_thread(std::string client_ip,
  ...
  if (!rdma_mg->pre_allocated_flag) {
    ...
    if (!rdma_mg->pre_allocated_flag) {
      ...
      // Broadcast metadata (base pointers and rkeys) for all logical regions
      // this memory node hosts
      for (const auto &[logical_id, group] : rdma_mg->logical_groups) {
        for (const auto &phys_reg : group.physical_regions) {
          if (phys_reg.phys_id == rdma_mg->node_id) {
            broadcastReplicaMetadata(logical_id, phys_reg.phys_id,
                                     phys_reg.base_ptr, phys_reg.rkey);
          }
        }
      }
    }
  ...
  memcpy(temp_send + sizeof(ibv_mr), rdma_mg->global_lock_table,
         sizeof(ibv_mr));
  ...
  if (rdma_mg->node_id == 1) {
    ...
    memcpy(temp_send + 2 * sizeof(ibv_mr), rdma_mg->global_index_table,
           sizeof(ibv_mr));
    rdma_mg->timestamp_oracle = rdma_mg->create_timestamp_oracle();
    memcpy(temp_send + 3 * sizeof(ibv_mr), rdma_mg->timestamp_oracle,
           sizeof(ibv_mr));
    printf("timestamp oracle sent to node%u is %p\n", compute_node_id,
           rdma_mg->timestamp_oracle->addr);
    fflush(stdout);
  }

  if (rdma_mg->sock_sync_data(
          socket_fd, 4 * sizeof(ibv_mr), temp_send,
    ...
    *receive_msg_buf = *(RDMA_Request *)recv_mr[buffer_position].addr;
    ...
    if (receive_msg_buf->command == create_mr_with_size_) {
      ...
      rdma_mg->post_receive<RDMA_Request>(&recv_mr[buffer_position],
                                          compute_node_id, client_ip);
      assert(false);
      //        create_mr_any_handler(receive_msg_buf, client_ip,
      //        compute_node_id);
      ...
      uint32_t target_node_id_pair =
          receive_msg_buf->content.qp_config_xcompute.node_id_pairs;
      qp_info_map_xcompute.insert(
          {target_node_id_pair, receive_msg_buf->content.qp_config_xcompute});
      if (qp_info_map_xcompute.size() ==
          rdma_mg->GetComputeNodeNum() * (rdma_mg->GetComputeNodeNum() - 1)) {
      ...
      rdma_mg->post_receive<RDMA_Request>(&recv_mr[buffer_position],
                                          compute_node_id, client_ip);
      //          rdma_mg->broadcast_new_root(receive_msg_buf, client_ip,
      //          compute_node_id);
      //        rdma_mg_->post_send<registered_qp_config>(send_mr, client_ip);
      //        rdma_mg_->poll_completion(wc, 1, client_ip, true);
...
void Memory_Node_Keeper::create_mr_1GB_handler(RDMA_Request *request,
  ...
  {
    std::unique_lock<std::shared_mutex> lck(rdma_mg->local_mem_mutex);
    assert(request->content.mr_request.mem_size ==
    ...
    try {
      if (!rdma_mg->Local_Memory_Register(&buff, &mr,
                                          request->content.mr_request.mem_size,
                                          Regular_Page, logical_region_id)) {
        fprintf(
            stderr,
            "Memory registering failed for logical region %u by size of 0x%x\n",
            logical_region_id,
...
//        {
//            std::unique_lock<std::shared_mutex> lck(rdma_mg->local_mem_mutex);
//            assert(request->content.mem_size = 1024*1024*1024); //
//            Preallocation requrie memory is 1GB if
//            (!rdma_mg->Local_Memory_Register(&buff, &mr,
//            request->content.mem_size,
//                                                Regular_Page)) {
//                fprintf(stderr, "memory registering failed by size of 0x%x\n",
...
//
//        rdma_mg->RDMA_Write(request->buffer, request->rkey, &send_mr,
//                            sizeof(RDMA_Reply), client_ip, IBV_SEND_SIGNALED,
//                            1, target_node_id);
//        rdma_mg->Deallocate_Local_RDMA_Slot(send_mr.addr, Message);
//        delete request;
//    }
...
void Memory_Node_Keeper::Get_qp_info_handler(RDMA_Request *request,
  ...
  auto target_qp_info =
      qp_info_map_xcompute.at(request->content.target_id_pair);
  assert(request->buffer != nullptr);
  assert(request->rkey != 0);
  ...
  //        send_pointer->content.qp_config.lid = rdma_mg->res->port_attr.lid;
  //        memcpy(send_pointer->content.qp_config.gid, &(rdma_mg->res->my_gid),
  //        16);
  send_pointer->received = true;
  Registered_qp_config *remote_con_data =
      new Registered_qp_config(request->content.qp_config);
...
void DSMEngine::Memory_Node_Keeper::broadcastReplicaMetadata(
    uint16_t logical_id, uint16_t physical_id, uint64_t base_ptr,
    uint32_t rkey) {
  // Use RDMA_Manager's memcached interface
  if (!rdma_mg) {
    fprintf(stderr, "Memory_Node_Keeper: RDMA_Manager not available for "
                    "memcached operation\n");
    return;
  }
  ...
  char key[100];
  snprintf(key, sizeof(key), "metadata_logical_%u_physical_%u", logical_id,
           physical_id);
