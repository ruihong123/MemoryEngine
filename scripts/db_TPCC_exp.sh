...
conf_file="../connection_replication.config"
memcached_conf_file_all=$bin/../memcached_db_servers.conf
...
launch () {
  ...
  # Get compute nodes from conf_file_all (connection_dbservers.conf) - skip comments and empty lines
  compute_line_all=$(grep -v '^#' "$conf_file_all" | grep -v '^$' | sed -n '1p')
  
  # Get memory nodes from conf_file_all (connection_dbservers.conf) - skip comments and empty lines
  ...
  # Copy memcached configuration from memcached_db_servers.conf to project directory
...
#       for thread_n in ${thread_number[@]}; do
#         compute_ARGS="-p$port -sf$ware_num -sf1 -c$thread_n -rde${FREQUENCY_DELIVERY[$qr_index]} -rpa${FREQUENCY_PAYMENT[$qr_index]} -rne${FREQUENCY_NEW_ORDER[$qr_index]} -ror${FREQUENCY_ORDER_STATUS[$qr_index]} -rst${FREQUENCY_STOCK_LEVEL[$qr_index]} -t8000000 -f${conf_file}"
#         run_tpcc_dist
#       done
#     done
#   done
