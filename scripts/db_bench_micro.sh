...
#            	port=`echo $memory | cut -d ' ' -f2`
          ...
          ssh -o StrictHostKeyChecking=no $ip	"ulimit -c 1000 && cd $BIN_HOME && numactl --physcpubind=31 ./memory_server_term  $port $(($remote_mem_size+2)) $((2*$i +1)) $remote_mem_size | tee -a $log_file.$ip " &
          sleep 1
          i=$((i+1))
#        	if [ "$i" = "$node" ]; then
#        		break
